...
controllers.writeChecklist = async (req, res) => {
  const { file, column } = req.body;
   try {
    const data = await Empreendimento.findOne({
      where: { id },
    });
    const dataExcel = await operacoesExcel.readExcel(data.excel);
    const dataDetails = { ...data.dataValues, ...dataExcel };
    //await operacoesExcel.writeCheckList(data.excel, 'J', null );
    res.status(200).json({ success: true, data: dataDetails, message: 'Detalhes obtidos com sucesso.' });
  } catch (error) {
    console.log("Error: " + error);
    res.json({ success: false, message: 'Ocorreu um erro ao obter os detalhes do empreendimento.' });
...
controllers.readChecklist = async (req, res) => {
  const { file, column } = req.body;
   try {
    const dataChecklist = await operacoesExcel.readCheckList(file, column);
    res.status(200).json({ success: true, data: dataChecklist, message: 'Checklist obtida com sucesso.' });
  } catch (error) {
    console.log("Error: " + error);
    res.json({ success: false, message: 'Ocorreu um erro ao obter a checklist do empreendimento.' });
  }
};

controllers.editChecklist = async (req, res) => {
  const { file, column } = req.body;
   try {
    const data = await Empreendimento.findOne({
      where: { id },
